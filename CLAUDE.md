...
│  ├ notion.js         ← 学習サマリを日記へ自動追記（pushDiary・Notion連携）
│  ├ store.js          ← Sheets読み書き
│  ├ fsrs.bundle.js    ← ts-fsrsバンドル（build/で生成）
│  └ setup.js          ← 初回セットアップ＋migrate（タブ/設定の後付け追加）
...
> 「詰まったら先生に聞く」（§5 ask）が1問1行で追記する。履歴画面（getHistory）と日記出力（pushDiary）の素材。**正解コードは含まない**（家庭教師プロンプトが担保）。migrate未実行でも ask 自体は動くよう、保存失敗は飲み込む
...
| diary_block_id / diary_block_date | — | pushDiaryが日記に入れたトグルのID/日付。同日の再実行で置き換え（重複防止） |
...
| `getHistory` | `limit`(既定30) | `{items:[…]}`（新しい順。各itemに問題文・自分のcode・verdict・hint_used・self_note・した質問） | 読み取りのみ。attempts に problems・asks を結合。履歴画面の素材 |
| `pushDiary` | なし | `{ok, message}`（未設定/失敗時もerror種別＋日本語で返す） | その日のattempts・asksから**コードがサマリを組み立て**（LLM不使用）、Notion日記の当日ページ「python学習」セクションへトグル1個で追記。同日再実行は古いトグルを消して置換（重複防止）。当日ページが無ければ作成。トークンは§9-2の通りScript Propertiesのみ・日記システムとは別トークン |
...
2. **最小権限（Least privilege）**：Gemini鍵・`NOTION_TOKEN` はGAS Script Propertiesのみ、フロントには秘密を一切置かない。`appsscript.json` の `oauthScopes` は `spreadsheets` と `script.external_request` の2つに限定明記。既存の日記GASとはプロジェクト・鍵・シートを完全分離（侵害の横展開を遮断）。日記書き込み用の `NOTION_TOKEN` は**日記DBだけに共有した専用インテグレーション**にし、既存の日記システムとも別トークンにする（pushDiaryが漏れても被害は日記DBの追記に限定）
