...
| action | 入力 | 出力 | 処理 |
|---|---|---|---|
| `getToday` | なし | 未回答問題リスト＋現在地サマリ（習得数・本日の復習対象・ストリーク日数・**今日のボトルネック**＝直近20件のattemptsの最頻error_pattern。無ければmistakes通算トップ） | 読み取りのみ。§5bヘルスチェックもここで |
...
> 解答の途中保存（下書き）は**サーバを使わずブラウザ内（localStorage）に保存**する。コード・もらったヒント・した質問を問題ごとに残し、同じ問題を開くと続きから再開できる。通信不要＝圏外でも保存でき、GAS側の変更も不要（§8-2）。採点完了時にその問題の下書きは破棄する
