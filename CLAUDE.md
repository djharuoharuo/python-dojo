...
### §19 追補（体感速度の本丸）
- **実行内シートキャッシュ**（store.js）：1リクエスト中の同一シート全読みを1回に（以前はconfigだけで5〜6回＝2〜5秒のムダ）。書き込み時にそのシートのキャッシュを破棄するので実行内でも常に正しい値。
- **fastチェーン**（`model_chain_fast`・既定 flash-lite 先頭）：ヒント/自由質問/一言提案/寛容採点(EiPE・和訳)/採点ヒント段階は軽量モデルで数秒短縮。問題生成とフル解説は品質優先で通常チェーンのまま。fast先頭モデルが普通に使えた時は model_last_used を書かない（無駄なシート書き込み防止）。
- **問題文の同送**（`clientPayload_`）：hint/ask はクライアントが問題文を同送しシート全読みを省く（正誤判定はサーバ保存が正のまま＝§9非侵害）。
- **経過秒数ティッカー**（app.js）：ヒント/質問/採点/生成の待ちに「（n秒経過）」を表示（黙って待たせない §14）。
