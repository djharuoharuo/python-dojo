...
function actionGenerate_(body) {
  ...
  // --- スロット決定（復習1問＋新規 or 練習中） ---
  var slots = pickSlots_(concepts, count, acc);
  if (slots.length === 0) {
    return { error: 'no_concept', message: '出題できる概念がありません。conceptsシートを確認してください' };
  }

  // --- 各スロットの種別・テーマ・バグ素材・番号を確定 ---
  ...
  var specs = slots.map(function (slot, i) {
    ...
    return {
      ...
      type: type,
      theme: pickTheme_(String(conf.theme_weights || '音楽:0.4,セキュリティ:0.3,日常:0.3')),
      // デバッグ問題に仕込むバグは mistakes 上位から選ぶ
